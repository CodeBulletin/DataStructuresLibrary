#pragma once
#include "../Nodes.hpp"
#include <functional>
#include <stdexcept>

namespace DS {
	template<typename T>
	class DoublyLinkedList {
	private:
		std::shared_ptr<DLLNode<T>> head = nullptr, tail = nullptr;
		int size = 0;

	public:
		DoublyLinkedList() = default;

		DoublyLinkedList(T Val) {
			tail = head = std::make_shared<DLLNode<T>>(new DLLNode<T>());
			head->value = Val;
			size++;
		}

		DoublyLinkedList(const DoublyLinkedList<T>& list) {
			std::shared_ptr<DLLNode<T>> Next = list.head;
			while (Next) {
				PushBack(Next->value);
				Next = Next->next;
			}
		}

		~DoublyLinkedList() {
			Clear();
		}

		void Clear() {
			std::shared_ptr<DLLNode<T>> current_ptr = head;
			while (current_ptr)
			{
				head = current_ptr->next;
				current_ptr = head;
			}
			head = tail = nullptr;
			size = 0;
		}

		void PushFront(const T& value) {
			if (!tail) {
				PushBack(value);
				return;
			}
			std::shared_ptr<DLLNode<T>> new_node(new DLLNode<T>());
			new_node->value = value;
			new_node->next = head;
			head->prev = new_node;
			head = new_node;
			if (!head->next) {
				tail = head;
			}
			size++;
		}

		void PushAfter(std::shared_ptr<DLLNode<T>> Node, const T& value) {
			std::shared_ptr<DLLNode<T>> new_node(new DLLNode<T>());
			new_node->value = value;
			new_node->next = Node->next;
			Node->next->prev = new_node;
			Node->next = new_node;
			new_node->prev = Node;
			if (Node == tail) {
				tail = new_node;
			}
			size++;
		}

		void PushBack(const T& value) {
			std::shared_ptr<DLLNode<T>> new_node(new DLLNode<T>());
			new_node->value = value;
			if (!tail) {
				tail = head = new_node;
			}
			else {
				tail->next = new_node;
				new_node->prev = tail;
				tail = new_node;
			}
			size++;
		}

		void PushBefore(std::shared_ptr<DLLNode<T>> Node, const T& value) {
			if (Node == head) {
				PushFront(value);
				return;
			}
			std::shared_ptr<DLLNode<T>> new_node(new DLLNode<T>());
			new_node->value = value;
			new_node->next = Node;
			Node->prev->next = new_node;
			new_node->prev = Node->prev;
			Node->prev = new_node;
			size++;
		}

		T PopFront() {
			std::shared_ptr<DLLNode<T>> Node = head;
			if (head == tail) {
				head = tail = nullptr;
				size = 0;
				return Node->value;
			}
			head = head->next;
			head->prev = nullptr;
			size -= 1;
			return Node->value;
		}

		T PopAfter(std::shared_ptr<DLLNode<T>> Node) {
			std::shared_ptr<DLLNode<T>> nNode = Node->next;
			if (Node == tail) {
				throw std::out_of_range("End of Linked List Cant Remove");
			}
			if (nNode == tail) {
				Node->next = nullptr;
				tail = Node;
				size -= 1;
				return nNode->value;
			}
			std::shared_ptr<DLLNode<T>> nnNode = Node->next->next;
			nnNode->prev = Node;
			Node->next = nnNode;
			size -= 1;
			return nNode->value;
		}

		T PopBack() {
			std::shared_ptr<DLLNode<T>> Node = tail;
			if (head == tail) {
				head = tail = nullptr;
				size = 0;
				return Node->value;
			}
			std::shared_ptr<DLLNode<T>> pNode = tail->prev;
			pNode->next = nullptr;
			tail = pNode;
			size -= 1;
			return Node->value;
		}

		T PopBefore(std::shared_ptr<DLLNode<T>> Node) {
			if (Node == head) {
				throw std::out_of_range("Cant Remove Before Head");
			}

			if (head->next == Node) {
				return PopFront();
			}

			std::shared_ptr<DLLNode<T>> pNode = Node->prev;

			std::shared_ptr<DLLNode<T>> ppNode = pNode->prev;
			ppNode->next = Node;
			Node->prev = ppNode;
			pNode->next = nullptr;
			pNode->prev = nullptr;

			size -= 1;
			return pNode->value;
		}

		T Pop(std::shared_ptr<DLLNode<T>> Node) {
			if (Node == head) return PopFront();
			if (Node == tail) return PopBack();
			Node->prev->next = Node->next;
			Node->next->prev = Node->prev;
			Node->next = nullptr;
			Node->prev = nullptr;
			return Node->value;
		}

		std::shared_ptr<DLLNode<T>> GetNodeAt(int n) {
			if (n < 0 || n >= size) {
				return nullptr;
			}

			std::shared_ptr<DLLNode<T>> Node = head;
			for (int i = 0; i < n; i++) {
				Node = Node->next;
			}
			return Node;
		}

		std::shared_ptr<DLLNode<T>> operator[](int n) {
			return GetNodeAt(n);
		}

		std::shared_ptr<DLLNode<T>> find(const T& Val) {
			std::shared_ptr<DLLNode<T>> Node = head;
			while (Node) {
				if (Node->value == Val) {
					return Node;
				}
				Node = Node->next;
			}
			return nullptr;
		}

		template<typename T2>
		std::shared_ptr<DLLNode<T>> findBy(const T2& Val) {
			std::shared_ptr<DLLNode<T>> Node = head;
			while (Node) {
				if (Node->value == Val) {
					return Node;
				}
				Node = Node->next;
			}
			return nullptr;
		}

		bool Empty() {
			return size == 0;
		}

		int Size() {
			return size;
		}

		std::shared_ptr<DLLNode<T>> Head() {
			return head;
		}

		std::shared_ptr<DLLNode<T>> Tail() {
			return tail;
		}
	};
}